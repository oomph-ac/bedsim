...
func TestSoulSpeedSkipsSoulSandSlowdown(t *testing.T) {
	...
	base := newBaseState()
	base.Pos = mgl64.Vec3{0.5, 1, 0.5}
	base.OnGround = true
	base.HasGravity = false

	without := *base
	(&Simulator{World: w, BlockSemantics: encodedBlockSemantics{}}).Simulate(&without, InputState{MoveVector: mgl64.Vec2{0, 1}})

	with := *base
	(&Simulator{World: w, BlockSemantics: encodedBlockSemantics{}, Equipment: fixedEquipment{EnchantmentSoulSpeed: 1}}).Simulate(&with, InputState{MoveVector: mgl64.Vec2{0, 1}})
...
func TestSwiftSneakAppliesAfterTwoSlowdownTicks(t *testing.T) {
	...
	state := newBaseState()
	input := InputState{SneakDown: true, MoveVector: mgl64.Vec2{0, 1}}

	sim.applyInput(state, input)
	if want := 0.3 * 0.98; math.Abs(state.Impulse.Y()-want) > 1e-12 {
		t.Fatalf("expected first-tick sneak impulse %v, got %v", want, state.Impulse.Y())
	}
	sim.applyInput(state, input)
	sim.applyInput(state, input)
	if want := 0.75 * 0.98; math.Abs(state.Impulse.Y()-want) > 1e-12 {
...
func TestItemUseAndInventoryActionInputRules(t *testing.T) {
	tests := []struct {
		...
		{name: "using item", input: InputState{UsingItem: true, MoveVector: mgl64.Vec2{0, 1}}, want: MaxConsumingImpulse * 0.98},
		{name: "using spear", input: InputState{UsingItem: true, UsingSpear: true, MoveVector: mgl64.Vec2{0, 1}}, want: 0.98},
		{name: "inventory action", input: InputState{InventoryAction: true, MoveVector: mgl64.Vec2{0, 1}}, want: 0},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if math.Abs(state.Impulse.Y()-tt.want) > 1e-12 {
