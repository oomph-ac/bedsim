...
func (s *Simulator) simulateMovement(state *MovementState) {
	...
	blockFriction := float32(DefaultAirFriction)
	...
	insideName := s.blockName(s.blockAtPos(posFromVec3(state.Pos)))
	...
	if state.SupportingBlockPos != nil {
		...
		if s.blockAir(blockUnder) {
			below := s.blockAtPos(posFromVec3(state.Pos).Side(dfcube.FaceDown))
...
func (s *Simulator) blockAtPos(pos dfcube.Pos) world.Block {
	if s.World == nil {
		return block.Air{}
	}
